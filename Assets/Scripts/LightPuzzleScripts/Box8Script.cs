...
public class Box8Script : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (GetComponent<Collider2D>().IsTouching(GameObject.FindWithTag("Player").GetComponent<Collider2D>()))
        {
            ...
            {
                Debug.Log("Box 8 pressed");
                // Flip all the switches
                affectedBoxes[0].GetComponent<Box5Script>().activated = !affectedBoxes[0].GetComponent<Box5Script>().activated;
